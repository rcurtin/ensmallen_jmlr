...
\ShortHeadings{The ensmallen library for flexible numerical optimization}{Curtin et al}
...
including \mbox{non-differentiable}, differentiable, separable, constrained, and categorical.
...
and libSVM~\citep{TODO}.
However, despite the abundance of numerical optimization toolkits,
none of them is flexible enough to support a wide range of use cases,
such as various types of objective functions.
...
including \mbox{non-differentiable}, differentiable, separable, categorical, constrained, and semidefinite.
Furthermore, custom behavior during optimization can be specified via {\it callback} functions,
for purposes such as printing progress, or inspection and modification of an optimizer's state.
...
The {\tt ensmallen} library also provides a large and intuitive set of pre-built optimizers;
...
The underlying framework facilitates the
implementation of new optimization techniques,
